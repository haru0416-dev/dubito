...
検証層の中で**すでに LP/MILP になっているもの**にだけ `verify()` を当てる。ルーターをナップサックにしない（クラス表であり、予算つき最適化ではない）。

| 内部計画 | いまの解き方 | 独立定式化 |
|---|---|---|
| 検証 IR の双対 / LP 緩和 | SciPy HiGHS 一本 (`dual.py`) | CVXPY/GLPK と OR-Tools/GLOP |
| 局所近傍 | 格子列挙 / Hypothesis | 半径 2 の箱を MILP にする |

`python -m dubito self`。IR からソルバーを生成しない原則は維持する。SciPy の双対解を独立定式化に代入して実行可能性を見る。
